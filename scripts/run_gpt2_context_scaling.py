"""GPT-2 Context Scaling: train at 2048, evaluate at 1024-8192.

Tests whether GPT-2's pretrained representations enable context-length
generalization that the tiny model couldn't achieve.
"""
...
SACCADIC_LAYERS = [6,7,8,9,10,11]; TRAIN_CTX = 2048
...
# ── Saccadic Components (same as multihop) ────────────────────────────────────

class PeripheralEncoder(nn.Module):
    def __init__(self, dim, bs):
        super().__init__()
        self.bs = bs
        self.w = nn.Linear(dim, 1); self.proj = nn.Linear(dim*3, dim)
...
class Foveal(nn.Module):
    ...
    def forward(self, x, fp, state, acc=None):
        ...
        ctx = ctx+out; cl = ctx[:,0]; cl = cl+self.ff(self.n2(cl))
...
class GPT2Saccadic(nn.Module):
    def __init__(self):
        ...
        for b in self.sblocks.values():
            for p in [b.pe, b.ctrl, b.fov, b.ma, b.mn, b.mg, b.op, b.on]:
                for pp in p.parameters(): pp.requires_grad = True
...
def main():
    ...
    log(f'Device: {device}')
    model = GPT2Saccadic().to(device)
    log(f'Trainable: {model.tp():,}')
    log(f'Training at {TRAIN_CTX} for {WALL_CLOCK}s...')
    train_model(model, device)
    results = []
    for ctx in EVAL_CONTEXTS:
        log(f'Evaluating at {ctx}...')
        acc, dist = evaluate(model, ctx, device)
        log(f'  {ctx}: accuracy={acc:.4f}, distance={dist:.2f}')
    ...
    with open('gpt2_context_scaling_results.json', 'w') as f:
